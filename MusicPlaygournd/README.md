...
See [DESIGN.md](DESIGN.md) for ownership and failure contracts. The host manifest pins published SwiftMusic 0.2.0. Build from the matching repository tag: the app bundles both source packages from that checkout for session evaluation when moved. This host remains under development; the library release does not mark pending editor UI acceptance complete.

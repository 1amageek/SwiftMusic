...
The implementation extends the existing `_SoundCompilationContext.visit` pass and `_SoundFragment`; it does not traverse a second lowered Sound tree. The current pass still evaluates each custom `Sound.body` once, validates and mutates the finite event result, and allocates each `CompiledSource`, `CompiledTrack` and render-graph node once. In parallel, each fragment captures one immutable internal `_EventProgram`: finite seeds, a rhythm/note generator with a child and resolved timed pattern, an ordered event modifier with a child, or a group of independent child programs. Track identity remains embedded in seeds and group composition does not merge source identities. This internal event-only program contains no render nodes and emission never calls `Sound.body` or allocates graph/source identity.

The existing `apply(_:to:sourceRange:)` remains the finite semantic authority and records the corresponding event-program operation after successfully applying that modifier. A first generator wraps the already resolved finite child seeds, so pre-generator gain/pan stays baked. Gain/pan after a generator records an onset sampler, so live emission re-evaluates future exact onsets. Offset and Sound fast/slow record exact event/time operations when recurrence exists. `analyze(_:)` computes checked natural periods and the common window; `emit(_:through:)` walks only the immutable event program and returns final events under the existing count limit.

For nested rhythm/note generators, analysis resolves the child for exactly one complete checked child natural period, then applies the outer generator in the same leaf-major, pitch-major, original-event order as finite compilation and composes the resulting period before horizon emission. It never feeds an unbounded child stream into an outer generator. Array `notes([Pitch])` remains an ordered finite assignment rather than a recurrence generator or independent clock: it assigns by the canonical current event order, the assigned pitches repeat with that enclosing generator template, and its array count does not lengthen the recurrence period. `NotePattern` remains the API for a pitch sequence whose own pattern period participates in live recurrence.
...
Offset preserves the same stage boundary. Applied to an active recurrence, it shifts that recurrence's canonical phase exactly and normalizes generated occurrences modulo its period, including the preceding occurrence needed for a boundary-crossing duration; it does not add a second stream. Applied before any generator, or after `oneShot()` has cleared recurrence, it remains the existing finite absolute delay and is never wrapped into the live window. Thus a delayed one-shot remains independent of recurring siblings and is emitted once only when its finite onset lies in the selected live window.

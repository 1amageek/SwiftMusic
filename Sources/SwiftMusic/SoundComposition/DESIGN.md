...
P05.4 adds domain-specific event modifiers rather than a public generic pattern/Strudel facade. `Swing` is a public `Sendable`, `Equatable`, `Hashable` descriptor with immutable `subdivision` and `delay`; `init(subdivision: MusicalTime = .eighth, delay: MusicalTime) throws` requires positive subdivision and `delay < subdivision`, with zero delay as the neutral exact branch. `Sound.swing(_:)` delays events whose zero-based subdivision cell is odd by the exact delay, leaves even cells unchanged and preserves duration. Finite extent grows only when a shifted event crosses it. In live compilation its clock is exactly two subdivisions and joins the existing checked common period; starts wrap within that period without changing event duration.

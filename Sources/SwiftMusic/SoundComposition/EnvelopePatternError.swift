/// Located failures while resolving a keyed envelope pattern.
public enum EnvelopePatternError: Error, Equatable, Sendable {
    case emptyInput
    case emptyTypedValues
    case emptyValueMap
    case emptyGroup(offset: Int)
    case invalidToken(token: String, index: Int, offset: Int = 0)
    case unknownKey(token: String, index: Int, offset: Int = 0)
    ...
    public var utf8Offset: Int? {
        switch self {
        case .emptyInput: 0
        case .emptyTypedValues, .emptyValueMap, .tooManyTypedValues, .zeroFactor, .invalidRate: nil

...
`@MainActor PerformanceObservationSession<Base>` owns one resolved `PerformanceMusic<Base>`, its supplied model references, a compiler, and one invalidation generation. Each `prepare(revision:liveLoop:)` validates requirements before body access and evaluates body under `withObservationTracking`, returning the ordinary nonthrowing `LiveMusicUpdate` success or typed compilation failure. The Observation callback is one-shot: it coalesces repeated changes into one MainActor notification for the current generation, and the host calls `prepare` again to re-arm tracking. `invalidate()` makes every outstanding callback inert and releases the session's model references; no callback compiles, renders, mutates `LiveMusicState`, or crosses an audio callback by itself.
